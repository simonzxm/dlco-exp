...
\hline
寄存器堆 (\texttt{regfile\_inst}) & RAMS32 $\times$ 8 & Distributed RAM（LUT） \\
\hline
...
\textbf{寄存器堆}使用了 8 个 \texttt{RAMS32}（Single-port Distributed RAM 原语），属于 Distributed Memory 类别。这是因为 Verilog 代码中使用了 \texttt{assign} 实现异步读取，这种组合逻辑行为只能用 LUT 来实现，Vivado 自动将其综合为分布式 RAM。8 个 \texttt{RAMS32} 各提供 32$\times$1 的存储，组合起来实现 16$\times$8 的地址空间（每个 RAMS32 负责 1 位输出数据）。
...
\hline
\texttt{top}（整体） & 27 & 8 & 17 & 1 \\
\hline
\texttt{regfile\_inst} & 13 & 8 & 0 & 0 \\
\hline
